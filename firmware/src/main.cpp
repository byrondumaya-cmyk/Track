...
static void logEvent(const char *fmt, ...) {
  ...
  char entry[160];
  snprintf(entry, sizeof(entry), "[%lus] %s",
...
// Wi-Fi setup form + live provisioning status — polls /status during connection attempt
...
bool tryConnectWiFi() {
  ...
  logEvent("WiFi: Connecting to %s (RSSI %d)", ssids[bestIdx].c_str(), bestRSSI);
...
void fetchDeviceConfig() {
  ...
  if (body.isEmpty()) { logEvent("CFG: Empty response"); return; }
  ...
  for (JsonObject net : networks) {
    ...
    if (ssid.length() > 0) { saveWiFiToNVS(ssid, pass); merged++; }
  }
...
bool verifyDeviceAuth() {
  ...
  // Send empty array — ingest returns 200/201 even with no records if auth is valid
  int code = http.POST("[]");
  http.end();
  logEvent("Auth check: HTTP %d", code);
  // 200/201 = auth OK; 400 (bad body) also means auth passed
  ...
  switch (cur) {
    ...
    case PROV_CONNECTING: {
      ...
      if (ws == WL_CONNECTED) {
        ...
        g_health.fail_reason = (ws == WL_NO_SSID_AVAIL)
                                 ? FAIL_SSID_NOT_FOUND : FAIL_WRONG_PASSWORD;
    ...
    case PROV_SUCCESS:
    case PROV_FAILED:
      // Terminal states — portal handles retry (FAILED) or normal operation (SUCCESS)
      break;
  }
}
...
String buildStatusJSON() {
  ...
  gp["last_fix_ago_s"]   = g_lastGpsFixMs > 0
                             ? (millis() - g_lastGpsFixMs) / 1000 : 0;
  ...
  cm["last_upload_ago_s"]   = g_lastUploadMs > 0
                                ? (millis() - g_lastUploadMs) / 1000 : 0;
  ...
  if (xSemaphoreTake(g_logMutex, pdMS_TO_TICKS(50)) == pdTRUE) {
    int start = (g_logCount >= LOG_MAX_ENTRIES)
                  ? g_logHead : (g_logHead - g_logCount + LOG_MAX_ENTRIES) % LOG_MAX_ENTRIES;
...
bool uploadRecord(GPSRecord &r) {
  if (g_wifiConnected) {
    if (uploadViaWiFi(r)) return true;
...
bool flashLoad(GPSRecord &out) {
  if (prefs.getInt(FLASH_KEY_COUNT, 0) == 0) return false;
  int head = prefs.getInt("head", 0);
  char key[8];
...
void TaskLED(void *pvParameters) {
  ...
  for (;;) {
    ...
    if (ps == PROV_IDLE) {
      ...
      digitalWrite(LED_PIN, HIGH); vTaskDelay(pdMS_TO_TICKS(2000));
      digitalWrite(LED_PIN, LOW);  vTaskDelay(pdMS_TO_TICKS(2000));
      ...
      for (int i = 0; i < 3; i++) {
        ...
        digitalWrite(LED_PIN, LOW);  vTaskDelay(pdMS_TO_TICKS(150));
      }
      ...
      if (g_consecutiveFailures > 3) {
        ...
        for (int i = 0; i < 2; i++) {
          ...
          digitalWrite(LED_PIN, LOW);  vTaskDelay(pdMS_TO_TICKS(120));
        }
  ...
  for (;;) {
    ...
    if ((xTaskGetTickCount() - lastPoll) >= pollInterval) {
      ...
      if (rec.gps_fix) {
        ...
        logEvent("GPS: Fix %.5f,%.5f %.1fkm/h bat:%d%% sats:%d",
...
void TaskUpload(void *pvParameters) {
  ...
  for (;;) {
    ...
    while (flashCount() > 0) {
      ...
      logEvent("Upload: Flash record (%d remaining) via %s",
               flashCount(), g_wifiConnected ? "WiFi" : "LTE");
      if (uploadRecord(rec)) {
        flashPop();
        ...
        if (g_consecutiveFailures >= UPLOAD_FAIL_REBOOT_LIMIT) {
          logEvent("Upload: %d consecutive failures — rebooting", UPLOAD_FAIL_REBOOT_LIMIT);
          esp_restart();
        }
        break;
      }
...
void TaskWiFi(void *pvParameters) {
  ...
  if (g_wifiConnected) {
    g_health.wifi_connected = true;
    g_health.active_iface   = IFACE_WIFI;
    // Mark provisioning success if we connected normally at boot
...
void setup() {
  ...
  if (savedGps > 0) logEvent("Flash queue: %d records from prev session", savedGps);

  // ── Start Permanent Service AP (WPA2) ─────────────────────
  // AP stays active indefinitely regardless of STA state.
  WiFi.mode(WIFI_AP_STA);
  WiFi.softAP(DIAG_AP_SSID, DIAG_AP_PASS, DIAG_AP_CHANNEL, 0, DIAG_AP_MAX_CON);
  ...
  // Fetch cloud WiFi list via LTE (non-blocking path after tasks launch)
  fetchDeviceConfig();
  esp_task_wdt_reset();
