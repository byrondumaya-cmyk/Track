...
class TinyGsmA7672X : public TinyGsmModem<TinyGsmA7672X>,
 ...
 protected:
  bool modemConnect(const char* host, uint16_t port, uint8_t mux,
    ...
    // Some SIMCom modules reject this command even though TCP keepalive is
    // optional for our use case. Treat it as best-effort and continue.
    sendAT(GF("+CTCPKA=1,2,5,1"));
    waitResponse(2000L, GF("OK" AT_NL), GF("ERROR" AT_NL),
                 GF("CME ERROR" AT_NL));

    if (ssl) {
      hasSSL = true;
      // Re-enable the documented SSL setup sequence, but keep the keepalive
      // probe best-effort because some modules reject it unexpectedly.
      sendAT(GF("+CSSLCFG=\"sslversion\",0,3"));
      waitResponse(5000L, GF("OK" AT_NL), GF("ERROR" AT_NL),
                   GF("CME ERROR" AT_NL));
      ...
      waitResponse(2000L, GF("OK" AT_NL), GF("ERROR" AT_NL),
                   GF("CME ERROR" AT_NL), GF("+CCHSTART: 0" AT_NL));
  ...
  size_t modemRead(size_t size, uint8_t mux) {
    ...
    if (hasSSL) {
      ...
      int16_t len = 0;
      int comma1 = res.indexOf(',');
      int comma2 = res.lastIndexOf(',');
      if (comma1 >= 0 && comma2 > comma1) {
        len = res.substring(comma1 + 1, comma2).toInt();
      }
      if (len <= 0) {
  ...
  size_t modemGetAvailable(uint8_t mux) {
    ...
    if (hasSSL) {
      ...
      int comma2 = res.lastIndexOf(',');
      if (comma1 >= 0 && comma2 > comma1) {
        result = res.substring(comma1 + 1, comma2).toInt();
      }
    } else {
